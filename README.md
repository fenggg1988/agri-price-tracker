...
| 水果 | 柑橘类 / 梨 / 桃 / 李子 | 🟡 参考价兜底（标注"暂无免费日度源"） | 生意社无鲜果数据、农业农村部官方指数 API 需鉴权，暂用参考价 |
| 水果 | 全国水果价格指数（代理） | 🟢 由代表品种构造 | 归一化均值、基准=100 的方向性代理指数（官方指数需 API 鉴权） |

> **水果数据源说明**：水果现货（尤其柑橘/梨/桃/李子）没有统一免费公开 API。
> - **苹果**走郑商所期货主连（AP0），与尿素同款实时抓取。
> - **全国水果价格指数（代理）**用监控水果价格的归一化均值构造（基准 100），用于"大概监控市场方向"；
>   若你拿到**农业农村部农产品批发价格 200 指数**的 API token，可在 `collect()` 中替换为一行官方真指数。
> - 柑橘/梨/桃/李子：经实测，生意社报价中心只有化工品（山梨醇、葡萄糖、扁桃酸…）、无鲜果报价，
>   农业农村部官方指数接口需鉴权（401）。因此这 4 个品种暂用 `TARGETS` 中人工维护的参考价占位，
>   看板标注"暂无免费日度源"。拿到可靠源后填 `pid`/接 API 即可自动升级为实时。
...
- 改 `FRUIT_BASELINE`（水果代理指数基线，基准=100）；
